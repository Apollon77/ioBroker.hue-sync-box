/**
 * Created by alex-issi on 09.12.22
 */
		...
		<React.Fragment>
			{collapse.active ? (
				...
				>
					...
					>
						{alertTitle ? <AlertTitle>{t(alertTitle)}</AlertTitle> : null}
						{text}
					</Alert>
				</Collapse>
			) : (
				<Alert variant={variant ? variant : 'filled'} severity={alertType} icon={icon} color={color} sx={sx}>
					{alertTitle ? <AlertTitle>{t(alertTitle)}</AlertTitle> : null}
					{text}
				</Alert>
			)}
