  ...
  async registration(obj) {
    ...
    try {
      ...
      if (registrations) {
        ...
        this.writeLog(
          `[Adapter v.${this.version} axios v.${import_axios.default.VERSION} registration] registration for ${device.name} was not successful - no response ${registrations}`,
          "error"
        );
      }
    } catch (error) {
  ...
  async requestRegistrationsId(obj) {
    try {
      ...
      if (registrations) {
        ...
        this.writeLog(
          `[Adapter v.${this.version} axios v.${import_axios.default.VERSION} requestRegistrationsId] request registrations id for ${device.name} failed no response => ${registrations}`,
          "error"
        );
      }
  ...
  async deleteRegistrations(obj) {
    try {
      ...
      if (deleteResponse) {
        ...
        this.writeLog(
          `[Adapter v.${this.version} axios v.${import_axios.default.VERSION} deleteRegistrations] delete registration for ${device.name} failed => no response => ${deleteResponse}`,
          "error"
        );
      }
    } catch (error) {
